...
namespace BencodeNET
{
    ...
    public class BencodeParser
    {
        ...
        public T Parse<T>(BencodeStream stream) where T : class, IBObject
        {
            Type type = typeof(T);
#if NETCORE
            var parseProc = CollectionExtensions.GetValueOrDefault(fParsers, type);
#else
        ...
        protected BString ParseStr(BencodeStream stream)
        {
            ...
            if (!TryParseLongFast(lengthString.ToString(), out long stringLength))
        ...
        protected BNumber ParseNumber(BencodeStream stream)
        {
            ...
            if (!TryParseLongFast(digits.ToString(), out long number)) {
